...
*------------------------*/
var gulpversion = '4' // указание версии gulp, если 3-я то нужно указать 3
...
// Если используется 3-я версия gulp
if (gulpversion == '3') {
...
// Если используется 4-я версия gulp
if (gulpversion == '4') {
 ...
 // Удаление продакшн директории
  ...
  // Запуск сборки проекта
  gulp.task('build', gulp.parallel('pug', 'styles', 'styles:vendor', 'scripts', 'scripts:vendor', 'fonts', 'images'));

   // Слежка за изменением файлов в проекте и если это произошло, то запускаются соответствующие задачи
  gulp.task('watch', function() {
    gulp.watch(path.pug.src, gulp.parallel('pug'));
    gulp.watch(path.sass.src, gulp.parallel('styles'));
    gulp.watch(path.js.src, gulp.parallel('scripts'));
    gulp.watch('./bower.json', gulp.parallel('styles:vendor', 'scripts:vendor'));
